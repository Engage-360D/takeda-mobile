...
NSString * const vkAppId = @"4289667";
...
- (void)getUserInfo
{
    ...
    [fields appendString:@"sex,bdate,country,contacts"];
...
- (void)getCountryByID:(NSString*)id_country{
    if (![self isAuthorized]) return;
    
    NSMutableString *requestString = [[NSMutableString alloc] init];
	[requestString appendFormat:@"%@/", @"https://api.vk.com/method"];
    [requestString appendFormat:@"%@?", @"database.getCountriesById"];
    [requestString appendFormat:@"country_ids=%@&", id_country];
    [requestString appendFormat:@"access_token=%@", accessToken];
    ...
    if ([parsedDictionary objectForKey:@"response"])
    {
        ...
        {
            [self.delegate vkontakteDidFinishGettinCountry:parsedDictionary];
        }
    }
    else
    {
